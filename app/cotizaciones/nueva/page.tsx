...
export default function NuevaCotizacion() {
  ...
  const [fichasTecnicas, setFichasTecnicas] = useState<{ titulo: string; descripcion: string; archivo: string }[]>([])
  ...
  function calcularFechaVencimiento(dias: number) {
  ...
  function numeroATexto(numero: number) {
  ...
  const seleccionarProducto = (id: number, productoId: string) => {
  ...
  const actualizarFichasTecnicas = () => {
    try {
      ...
      const todasFichas: ((prevState: never[]) => never[]) | { titulo: string; descripcion: string; archivo: string }[] = []
  ...
  const actualizarCertificadosCombinados = () => {
    try {
      ...
      let todosCertificados: string | any[] = []
      codigosUnicos.forEach((codigo) => {
        const producto = productosPreexistentes.find((p) => p && p.id === codigo)
  ...
  const generarCertificadosTexto = (certificados: any[]) => {
    if (!certificados || certificados.length === 0) return ""

    // Usar formato de texto plano para el textarea
    return certificados
      .map(
        (cert: { titulo: any; codigo: any; tipo: any; informe: any; detalle: any[] }) =>
          `${cert.titulo}\n` +
  ...
  const actualizarItem = (id: number, campo: string, valor: string | number) => {
    try {
      setItems(
        items.map((item) => {
          if (item.id === id) {
